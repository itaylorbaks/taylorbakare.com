document.addEventListener('DOMContentLoaded', function() {
    const blockSlider = new Swiper('.js-block-slider',{
        speed: 4000,
        grabCursor: !0,
        slidesPerView: 'auto',
        freeMode: !0,
...
;document.addEventListener('DOMContentLoaded', function() {
    const layerTop = document.querySelector('.js-block-tags-title-layer-top');
    const layerBottom = document.querySelector('.js-block-tags-title-layer-bottom');
    const title = document.querySelector('.block-tags__title');
    const titleWidth = document.querySelector('.block-tags__title').offsetWidth;
    window.locoScroll.on('call', (e) => {
        if (e === 'block-spotlight-call') {
            document.addEventListener('mousemove', (e) => {
                let mouseTargetClass = e.target.className;
                if (mouseTargetClass === 'block-tags__title block-title is-inview') {
                    let clientMoveX = e.clientX;
                    let percentX = 100 / (titleWidth / clientMoveX);
                    let percentXMask = (titleWidth / 100) * percentX;
                    let percentY = (e.clientY - (window.pageYOffset + title.getBoundingClientRect().top));
                    layerTop.style.webkitMaskPosition = `calc(${percentXMask}px - 1.650em) calc(${percentY}px - 1.900em)`;
                    layerBottom.style.transform = `translate3d(calc(${percentX}% - 0.800em), calc(${percentY}px - 0.550em), 0)`
                }
                else if (mouseTargetClass === 'block2-tags__title block2-title is-inview') {
                    let clientMoveX = e.clientX;
                    let percentX = 100 / (titleWidth / clientMoveX);
                    let percentXMask = (titleWidth / 100) * percentX;
                    let percentY = (e.clientY - (window.pageYOffset + title.getBoundingClientRect().top));
